...
import QuizData from "../Data/QuizData"; // Ensure you have this data correctly imported
...
function Quiz() {
  ...
  return (
    <div className="h-max  md:h-screen flex items-center justify-center px-4 md:px-8 lg:px-20 py-4 md:py-8">
      <div className="flex flex-col bg-[#1B1C26] w-full max-w-[80vw] rounded-3xl h-max py-16 md:py-0 md:h-[80vh] p-6 md:p-10 lg:p-16 items-center place-content-center relative">
        {!startQuiz ? (
          <div className="flex flex-col items-center w-full justify-center text-[#FFFFFF] gap-7">
            ...
            <button
              ...
              className="bg-[#74C7D0] text-black px-3 md:px-8 text-sm md:text-3xl font-semibold py-2 md:py-4 rounded-2xl hover:bg-[#64abb3] transition duration-300"
            >
              Start Quiz
            </button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center text-[#FFFFFF] gap-7">
            ...
            <button
              onClick={restartQuiz}
              className="bg-[#74C7D0] text-black px-6 md:px-8 text-lg md:text-3xl font-semibold py-2 md:py-4 rounded-2xl hover:bg-[#64abb3] transition duration-300 my-10"
            >
              Try Again
            </button>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-1 md:gap-5 w-full">
                {questions[currentQuestion]?.options.map((option, index) => (
                  <button
                    ...
                    className="bg-[#74C7D0] text-black px-1 text-xs md:text-2xl font-semibold py-1 md:py-3 rounded-2xl hover:bg-[#64abb3] transition duration-300 w-full"
                  >
                    {option}
                  </button>
                ))}
        ...
        <div className="absolute bottom-0 left-0">
          <FontAwesomeIcon
            ...
            className="text-5xl md:text-[100px]"
          />
        </div>
      </div>
    </div>
